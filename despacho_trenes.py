...
def bodega_descontar(env, camino, quantity):
    ...
    camino.cambio_proyeccion(- quantity, env.now)
    yield env.timeout(0)
...
def despacho_trenes(env, camino, quantity_out, quantity_in):
    ...
    camino.cambio_proyeccion(- quantity_out, env.now)
...
def tramo_trenes(env, camino, file):
    while True:
        ...
        if aleatorio <= 0.05:
            ...
            quantity = int(min(max(0, camino.origen.bodega), camino.proyection_per_day(env.now), 740))
            env.process(despacho_trenes(env, camino, quantity, quantity))
            yield env.timeout(12)
        elif aleatorio <= 0.25:
            file.write("LLega 2 tren en el tiempo {}\n".format(env.now))
            yield env.timeout(6)
            quantity1 = int(min(max(0, camino.origen.bodega), camino.proyection_per_day(env.now), 740))
            env.process(bodega_descontar(env, camino, quantity1))
            yield env.timeout(8)
            quantity2 = int(min(max(0, camino.origen.bodega), camino.proyection_per_day(env.now), 740))
            ...
            yield env.timeout(4)
            quantity3 = int(min(max(0, camino.origen.bodega), camino.proyection_per_day(env.now), 740))
            ...
            yield env.timeout(4)
            quantity4 = int(min(max(0, camino.origen.bodega), camino.proyection_per_day(env.now), 740))
