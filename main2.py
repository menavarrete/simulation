...
def tramo_vuelta_camion(env, camino, camion, camiones, file):
    ...
    file.write("llegada de un camion vacio desde {} a {} a las {}".format(camino.destino.name, camino.origen.name, env.now))


def tramo_ida_camion(env, camino, camion, camiones, file):
    camino.origen.sale_camion(camion)
    yield env.timeout(camino.tiempo_viaje)
    file.write("llega camion desde {} a {} con {} kilos a las {}".format(camino.origen.name, camino.destino.name, camion.carga, env.now))
...
def carga(camino, camion):
    camion.carga_camion(truck_capacity)
    camino.cambio_proyeccion(-truck_capacity)


def tramo_despacho_5(env, camino, between_time, file):
    while True:
        if camino.proyeccion >= truck_capacity:
            camion = Camion(camino, 0, truck_capacity)
            carga(camino, camion)
            env.process(tramo_ida_camion(env, camino, camion, [], file))
            file.write("Despacho de un camion desde {} a {} a las {} ".format(camino.origen.name, camino.destino.name, env.now))
            yield env.timeout(between_time)
        else:
            file.write("No hay suficiente materia prima en {} para ruta {}".format(camino.origen.name, camino.name))
            camino.no_salieron()
            yield env.timeout(between_time)
...
def tramo_despacho(env, camino, camiones, between_time, file):
    while True:
        if len(camiones) > 0 and camino.origen.bodega >= truck_capacity and camino.proyeccion >= truck_capacity:
            camion = camiones.pop()
            carga(camino, camion)
            ...
            if len(camiones) < 1:
                file.write("No hay camiones disponibles en {} para ruta {}".format(camino.origen.name, camino.name))

            elif camino.origen.bodega < truck_capacity:
                file.write("No hay suficiente materia prima en bodega en {} para ruta {}".format(camino.origen.name, camino.name))

            else:
                file.write("Se supera la proyeccion para ruta {}".format(camino.name))
            camino.no_salieron()
            yield env.timeout(between_time)
...
if __name__ == '__main__':
    ...
    # Tramos
    tramo5 = Camino(tramo5_name, tramo5_travel_time, teniente_source, andina_bodega, 221000)
    tramo4 = Camino(tramo4_name, tramo4_travel_time, andina_bodega, ventanas, 73000)
    tramo1 = Camino(tramo1_name, tramo1_travel_time, saladillo_bodega, ventanas, 46000)
